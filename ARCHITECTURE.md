...
This project has two distinct phases with different deployment targets. **The application architecture (API, game loop, data model, Socket.IO) is identical in both phases.** Only the hosting layer changes.
...
│         Socket.IO (WebSocket) + REST API calls              │
└──────────────────────────┬──────────────────────────────────┘
                           │
                     HTTPS / WSS
                           │
...
| Layer | Choice | Notes |
|-------|--------|-------|
| **API Server language** | TypeScript (strict mode) | NestJS on Node.js LTS |
| **Worker language** | Go | Standalone binary. No Node.js runtime. Ideal for tick loop, concurrent simulation, and Redis I/O. |
| **Backend framework** | NestJS (API Server only) | Module/decorator/DI structure maps to ASP.NET Core patterns; widely used in enterprise Node.js stacks |
| **Real-time** | Socket.IO | Native WebSocket/long-poll fallback; room-based broadcasting. API Server only — Worker never speaks to clients. |
| **ORM (API Server)** | Prisma | Typed schema, migration-based, query client for PostgreSQL. Schema is the single migration source of truth. |
| **DB driver (Worker)** | pgx | Raw SQL for narrow, explicit game-state flush writes. No ORM in the Worker. |
| **Redis client (API Server)** | node-redis | Intent queue writes, hot state reads, pub/sub subscribe, Socket.IO adapter |
| **Redis client (Worker)** | go-redis | Intent queue drain, hot state read/write, pub/sub publish |
| **Auth** | Passport.js (OAuth2 strategy) + JWT | Google and Discord providers. Auth lives entirely in the API Server — Worker has no auth concerns. |
| **Frontend** | React + TypeScript | Shares `shared/types/` with API Server |
| **Container** | Docker | API Server and Worker are separate images |
| **IaC (Phase 2)** | AWS CDK (TypeScript) | Same language as API Server; strong portfolio signal |
...
**API Server** (`src/server`) — TypeScript · NestJS
- Auth (Passport.js OAuth + JWT)
- REST API — all player commands (build, buff), player profile, admin endpoints
- Socket.IO gateway — server-to-client push only (see rationale below)
...
- The Socket.IO adapter namespace (for multi-instance Phase 2)
...
| **REST API** | API Server | TypeScript | Authentication, player actions (build, buff) → enqueues intent to Redis, player profile/data, admin |
| **Socket.IO Gateway** | API Server | TypeScript | Server-to-client push only. Subscribes to worker events via Redis pub/sub and broadcasts to connected clients. |
...
| **Redis** | Shared | — | Intent queue, hot game state, pub/sub channel, Socket.IO adapter |
| **PostgreSQL** | Shared | — | Persistent storage: accounts, world definition, action history, capture log |

> **Note on Redis:** Hot game state, the pub/sub channel, the intent queue, and the Socket.IO adapter all share a single Redis instance. Logical separation is handled by key prefixes and channel namespacing. Separate clusters are a Phase 2+ concern.
...
| **Prisma owns migrations** | All schema migrations are generated via Prisma. The Worker's `pgx` writes must stay manually aligned with the Prisma schema. Migrations run in one place only. |
...
Player POSTs /api/game/build
  ...
  → API Server subscriber receives event → broadcasts to Socket.IO clients
```
...
Redis pub/sub is **notifications only**. The API Server receiving a pub/sub message should never need to take a write action as a result — it reads the payload and broadcasts it to Socket.IO clients. That's the entire job.
...
### Socket.IO is Push-Only — Why This Matters

All player commands go through the REST API. Socket.IO is strictly server-to-client broadcast. This is an explicit design decision, not an oversight.

**What this buys:**
- **Auth and validation stay in one place.** Every action passes through HTTP middleware — JWT verification, cooldown checks, rate limiting — before anything touches game state. Nothing slips through via a socket event.
...
- **Simpler socket code.** The gateway has one job: listen to Redis pub/sub and relay events to the right clients. No command parsing, no session state, no ack complexity.
...
The game loop runs in the **Worker process** as a `time.Ticker`-based scheduler (Go) on a configurable tick interval (e.g., every 5–10 seconds). It is isolated from the API server and has no knowledge of Socket.IO connections or HTTP requests.
...
7. **Publish** a tick result event to the Redis pub/sub channel (the API server picks this up and fans out to Socket.IO clients)
...
- PostgreSQL writes happen on a controlled interval via Prisma, making DB load predictable regardless of player count
...
**API Server** uses **Prisma** for all PostgreSQL interaction. Schema is defined in `schema.prisma`, migrations are managed via `prisma migrate`, and Prisma Client provides typed queries. Prisma is the single source of truth for schema migrations.

**Worker** uses **`pgx`** (raw SQL) for its narrow, explicit game-state flush writes. No ORM. The Worker's SQL paths are minimal and must stay manually aligned with the Prisma schema. When migrations run, the Worker's queries must be reviewed for compatibility — this is a small, explicit maintenance surface, not an automatic one.
...
```
Authentication
  GET  /api/auth/login/:provider      — Initiate OAuth flow (Google/Discord) via Passport.js
...
**Client → Server:** None. All actions go through the REST API. Socket.IO is push-only from the server.

> **Socket.IO adapter (Phase 2):** When running multiple API server instances behind a load balancer, the `@socket.io/redis-adapter` package enables cross-instance broadcasting — all instances share the same pub/sub channel and any instance can deliver an event to any connected client. Already accounted for in the Redis architecture.
...
1. Build & test Node.js apps (tsc, jest)
2. Build api-server Docker image, push to GHCR
3. Build worker Docker image, push to GHCR
4. SSH into VPS
5. docker compose pull && docker compose up -d
6. Build React app, rsync static files to VPS
```
...
│          REST API + Auth + Socket.IO Gateway                 │
└───────┬──────────────────────────────────┬──────────────────┘
        │ enqueues intent / reads cooldowns │ subscribes to pub/sub
        │                                  │
┌───────▼──────────────────────────────────▼──────────────────┐
│                    Redis (ElastiCache)                        │
│   Intent queue · Hot game state · Pub/sub · Socket.IO adapter│
└───────┬──────────────────────────────────┬──────────────────┘
...
Two ECS services, both managed under the same ECS cluster. The Worker service runs a single task (desired count = 1) — the tick loop must not run in parallel. The API service can scale horizontally behind an ALB when multi-instance support is needed; the Socket.IO adapter handles cross-instance broadcasting automatically.
...
- **API service scales horizontally** behind an ALB with sticky sessions (Socket.IO requires this for WebSocket handshake). The Socket.IO adapter handles cross-instance broadcasting automatically.
...
```
On push to main:
  1. Build & test API Server (tsc --noEmit, jest)
...
/
├── src/
│   ├── server/                         — API Server (NestJS)
│   │   ├── app.module.ts
│   │   ├── main.ts
│   │   ├── auth/
│   │   │   ├── auth.module.ts
│   │   │   ├── auth.controller.ts      — OAuth callback routes
│   │   │   ├── jwt.strategy.ts
│   │   │   └── passport/               — Google + Discord strategies
│   │   ├── player/
│   │   │   ├── player.module.ts
│   │   │   └── player.controller.ts    — Profile, cooldowns, contribution history
│   │   ├── game/
│   │   │   ├── game.module.ts
│   │   │   ├── game.controller.ts      — Build, buff, game state reads
│   │   │   └── game.gateway.ts         — Socket.IO gateway (push-only)
│   │   ├── admin/
│   │   │   ├── admin.module.ts
│   │   │   └── admin.controller.ts     — Stats, announce, world pause/resume
│   │   └── common/
│   │       ├── redis.service.ts        — node-redis client wrapper
...
│   └── migrations/
│
├── shared/                             — Shared TypeScript types (API Server + React client only)
│   └── types/
│       ├── game-state.ts               — Fortress, building, buff interfaces
│       └── events.ts                   — Pub/sub event shape definitions (TS side of Redis contract)
│
│   NOTE: The Go Worker defines equivalent structs in worker/state/ and worker/publisher/.
│   The Redis pub/sub JSON payload is the contract boundary between Go and TypeScript.
│   Keep shared/types/events.ts and the Go pub/sub structs manually in sync.
│
...
- [DONE] Repository setup, Docker Compose for local dev
- [DONE] NestJS project scaffold with Prisma + PostgreSQL
- [DONE] Redis integration with node-redis (API Server)
- [DONE] OAuth login (Google + Discord) via Passport.js
- [DONE] Basic REST API: player profile, game state read
- [DONE] Worker Go module setup (`go.mod`, go-redis, pgx dependencies)
- [DONE] Worker process scaffold (main.go, time.Ticker stub, Redis connection)
- [IN PROGRESS] Intent queue stub: API Server RPUSH, Worker LRANGE/DEL
- [ ] Caddy + Docker Compose production config (api-server + worker services)
- [ ] GitHub Actions CI/CD: separate build steps for TS and Go → GHCR → VPS deploy
...
- [ ] API Server: broadcasts worker-originated update to Socket.IO clients
...
- [ ] Socket.IO gateway (single instance — no adapter needed at this scale)
...
- [ ] Enable Redis Socket.IO adapter for multi-instance ECS deployment
...
| Real-time | Socket.IO + Redis adapter | Native Node.js, widely deployed, scales horizontally with Redis |
| Socket.IO push-only | Deliberate | All commands go through REST; socket is broadcast only — cleaner auth, validation, audit trail |
| Worker / API split | Two separate processes | Fault isolation, cleaner mental model, independent scaling path |
| Worker language | Go | Tick loop, concurrent simulation, and Redis I/O are ideal Go workloads. API Server stays TypeScript to share types with React and retain NestJS/Passport ecosystem. |
...
| Intent queue pattern | Redis List (`intents:pending`) | Decouples player input timing from simulation tick. API Server RPUSHes; Worker drains at tick start. Simple, inspectable, crash-safe. |
...
| Game loop | Standalone Go worker with time.Ticker | Isolated from HTTP/socket concerns; crash-safe, independently restartable, goroutine-native concurrency |
| IaC (Phase 2) | AWS CDK (TypeScript) | Same language as API Server, strong portfolio signal |
| ORM (API Server) | Prisma | Typed schema, migration-based, single source of schema truth |
| DB driver (Worker) | pgx | Narrow, explicit writes. No ORM needed for game state flush. |
| Auth | Passport.js OAuth2 + JWT | Standard Node.js auth stack; Google and Discord providers. Auth lives entirely in API Server. |
